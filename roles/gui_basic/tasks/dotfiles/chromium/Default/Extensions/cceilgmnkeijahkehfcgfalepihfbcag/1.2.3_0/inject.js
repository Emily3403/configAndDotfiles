(() => {
  ...
  var cachedWords = new Map(), observer = null, aliveName = null, deadName = null, newWords = [], oldWords = [], revert = !1, highlight;
  ...
  function replaceText(text, isTitle) {
    ...
    oldWords = oldWords.map((oldText) => oldText.toLowerCase()), highlight && !isTitle && (revert ? oldWords = oldWords.map((text2) => `<mark replaced="">${text2}</mark>`) : newWords = newWords.map((text2) => `<mark replaced="">${text2}</mark>`));
